...
export default function Today() {
  ...
  const unit_t = data?.hourly_units?.temperature_2m;
  const unit_wind = data?.hourly_units?.wind_speed_10m;
  const parseData = data?.hourly?.time?.map((item: any, index: number) => {
    return {
      time: item,
